...
func D05P2() {
	...
	for i := len(moves) - 1; i >= 0; i-- {
		...
		// Get crates to move
		cratesToMove := stacks[currentMove.from-1][len(stacks[currentMove.from-1])-currentMove.count:]
		// Remove crates from stack
		stacks[currentMove.from-1] = stacks[currentMove.from-1][:len(stacks[currentMove.from-1])-currentMove.count]
		// Add crates to stack
		stacks[currentMove.to-1] = append(stacks[currentMove.to-1], cratesToMove...)
	}
