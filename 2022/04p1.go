...
func D04P1() {
	...
	for _, assignmentPair := range assignmentPairs {
		if assignmentPair["elf1RangeStart"] <= assignmentPair["elf2RangeStart"] &&
			assignmentPair["elf1RangeEnd"] >= assignmentPair["elf2RangeEnd"] {
			totalOverlaps++
		} else if assignmentPair["elf1RangeStart"] >= assignmentPair["elf2RangeStart"] && assignmentPair["elf1RangeEnd"] <= assignmentPair["elf2RangeEnd"] {
			totalOverlaps++
		}
	}
